...
| Command                                                                            | Purpose                                                                   |
| ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| `hscli conv list --source <type>`                                                  | Filter conversations by `source.type`                                     |
| `hscli conv export --source <type>`                                                | Same filter on bulk export                                                |
| `hscli report beacon [--since N] [--mailbox]`                                      | Aggregate convs by `source.type`/`source.via` with percentages            |
| `hscli beacon sign --email --secret`                                               | HMAC-SHA256 of email for Secure Mode                                      |
| `hscli beacon verify --email --secret --signature`                                 | Verify a candidate signature (exit 0 on match, 1 on mismatch)             |
| `hscli beacon embed <id> [--color] [--position] [--style] [--text] [--icon-image]` | Generate `<script>` embed block                                           |
| `hscli beacon identify-snippet --beacon-id --secret --stack`                       | Generate server-side identify code (node / rails / php / django / python) |
...
- **Webhook events** — Beacon emits HS webhooks (`chat-started`, `email-sent`,
  `message-clicked`). Subscribe via existing `hscli webhook create/list` commands.
- **Source-derived analytics** — `hscli report beacon` aggregates conversations by
...
```bash
hscli beacon embed YOUR_BEACON_ID --color "#5b21b6" --position right --style iconAndText --text "Help"
```
...
hscli beacon identify-snippet --beacon-id YOUR_BEACON_ID --secret KEY --stack python
```
...
hscli report beacon --output json | jq .
```
...
- **HMAC algorithm.** Help Scout Beacon Secure Mode uses HMAC-SHA256 with hex
  ...
  payload). `hscli beacon sign` and `hscli beacon verify` match this exact contract.
...
- [`hscli webhook` commands](commands.md#webhooks) for Beacon-emitted events

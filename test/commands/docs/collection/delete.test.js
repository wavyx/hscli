...
const { default: Cmd } =
  await import('../../../../src/commands/docs/collection/delete.js')
const DOCS = 'https://docsapi.helpscout.net'
const ID = '5214c83d45667acd25394b53'

describe('hs docs collection delete', () => {
  afterEach(() => {
    nock.cleanAll()
    vi.clearAllMocks()
  })
  ...
  it('deletes without prompting when --yes is passed', async () => {
    const scope = nock(DOCS).delete(`/v1/collections/${ID}`).reply(204)
    const out = await runCmd(Cmd, [ID, '--yes'])
    expect(out).toContain(`Deleted collection ${ID}`)

...
:::note
`hscli docs` is read-focused today — `list`, `get`, and `search`. Article and collection
writes (create, update, drafts) are on the roadmap. The Docs API is rate-limited per
10-minute window; large pulls back off automatically on `429`.
:::
